...
/**
 * 初始化加载玩家数据
 * 该方法只能在游戏开始的时候调用一次，不能在别的地方再次被调用，如果只要获取玩家最新的数据，请调用get-current-player-data
 ...
 *  获取分辨率
 */
ipcMain.handle("get-screen-resolution", (event: IpcMainInvokeEvent): Response<{width: number, height: number, scaleFactor: number}> => {
